...
function useFlagStatus({ featureName, feature, variantList, handleProductionChange, enabledProd }) {
    ...
    useEffect(() => {
        var divArray = <div key={`${feature.name}-${Math.random()}`} className='flexStyle'>
                    ...
                    return (
                        <Suspense fallback={'Loading . . . '}>
                            <div className="col-7 individualUI" key={`${feature.name}-${Math.random()}`}>
                                {/* {checkEnabled ? */}
                                <HeaderMFE key={`${feature.name}-${Math.random()}`} url={data.prod_url + '/remoteEntry.js'} scope={data.scope} module={data.module} />
                                {/* :
            ...
            <div className='gridStyle col-3 individualUI' key={`${feature.name}-${Math.random()}`}>
                <div key={`${feature.name}-${Math.random()}`}>{enabledProd ? "ON" : "OFF"}</div>
                <Switch
                    checked={enabledProd}
